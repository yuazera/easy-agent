...
- 新增离线 `mock` provider 以及 `setup`、`init`、`quickstart`、场景模板、connector diagnostics、task packs、`config doctor`、`runs explain`、`traces open`、`report latest`、`report trend`、单文件 HTML trace/report export 和轻量 Python `AgentApp` facade，可以零凭据完成上手、配置理解和失败归因。
...
- 继续用 guided setup preflight、config explanation、connector diagnostics、task packs，以及面向 coding、research、data、ops、browser planning、support、sales、documents、QA、release checks 的业务模板固化零凭据上手层
