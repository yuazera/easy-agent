...
- 新增离线 `mock` provider 以及 `setup`、`wizard`、`init`、`quickstart`、场景模板、connector diagnostics、task packs、workflow packs、`config doctor`、`runs explain`、advice-only `runs triage` / `runs fix`、`traces open`、`report latest`、`report trend`、单文件 HTML trace/report/fix/dashboard export 和轻量 Python `AgentApp` facade，可以零凭据完成上手、配置理解和失败归因。
...
- 继续用 guided setup/wizard preflight、config explanation、connector diagnostics、workflow packs、browser smoke/snapshot/report helpers、browser doctor/artifact inspection、task packs、静态 dashboard suggestions、advice-only triage/fix packages，以及面向 coding、research、data、ops、browser automation、web monitoring、SEO、competitor research、meeting notes、content pipelines、support、sales、documents、QA、release checks 的业务模板固化零凭据上手层
