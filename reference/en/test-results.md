...
- The latest unreleased verification refresh on April 30, 2026 keeps the April 14 benchmark and headline public-eval score snapshot, while refreshing Python verification and workflow/browser/dashboard/failure-to-fix CLI checks. The live provider compatibility matrix remains the April 20 verification snapshot.
...
- Targeted regressions around setup preflight, wizard, config explanation, config doctor, searchable HTML trace export, local trace opening, latest-report summarization and HTML export, static dashboard export, suggested dashboard next steps, dashboard workflow/template recommendations, report trend, experimental OTel trace export, connector diagnostics, workflow packs, workflow YAML init/run, run triage/bundle, Playwright MCP browser readiness, browser doctor/artifact/smoke/snapshot/audit/report helpers, browser task packs, local skill catalog, plugin doctor, Python facade embedding, mock provider, onboarding CLI, scenario creation, new business templates, run explanation/fix packages, provider compatibility, config validation, guardrails, BFCL evaluation, official-source search, and simple-evals profile support: latest focused set `29 passed`; retained compatibility sets `55 passed`, `23 passed`, `36 passed`, `89 passed`, and `4 passed`
- Full unit coverage: `231 passed`
...
- New focused regressions cover setup preflight, wizard scenario creation, template `workflow.yml` generation, browser template Playwright MCP config, browser doctor/artifact/smoke/snapshot/audit/report helpers, browser task packs, workflow packs, workflow YAML init/run, run triage, run bundle evidence export, config explanation, config doctor, searchable HTML trace export, `traces open`, `report latest`, enhanced `dashboard`, dashboard workflow/template recommendation sections, `report trend`, latest-report HTML export, experimental OTel trace export, connector diagnostics, local skill catalog install, plugin doctor, offline mock runs, starter templates, `new <scenario>` for coding/research/data/ops/browser/web-monitor/SEO/competitor-research/meeting-notes/content-pipeline/support/sales/document/QA/release starters, Python `AgentApp`, quickstart, run explanation, advice-only JSON/Markdown/HTML run fix packages, browser-MCP failure classification, run listing, run summary, structured trace tree export, executor capability reports, storage contracts, and real-network scenario proof metadata.
- CLI smoke passed for `workflow list`, `workflow init`, `workflow run workflow.yml --dry-run`, `workflow run --dry-run`, `runs triage`, `runs bundle`, enhanced `dashboard`, `browser smoke`, `browser snapshot`, `browser audit`, `browser report`, `browser doctor`, `browser artifacts`, `connectors doctor`, `connectors test browser`, `task show repo-review`, `task show browser-qa`, `report trend`, `skills catalog list`, and `new web-monitor-agent`.
